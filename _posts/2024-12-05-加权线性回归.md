...
**模拟零假设**：模拟不相关的 $X$ 和 $Y$，因此理论上 $Y/U$ 和 $X/U$ 也不相关。

**结果比较**：比较不同权重 (包括 model1:一般线性回归、 model2: 加权线性回归2($\lvert u_i\rvert$) 和 model 3: 加权线性回归3($u_i^2$)) 下的斜率估计值和P值统计量：

斜率：

![image-20241205225712499](2024-12-05-%E5%8A%A0%E6%9D%83%E7%BA%BF%E6%80%A7%E5%9B%9E%E5%BD%92.assets/image-20241205225712499.png)

斜率对应的P值统计量：

![image-20241205225832571](2024-12-05-%E5%8A%A0%E6%9D%83%E7%BA%BF%E6%80%A7%E5%9B%9E%E5%BD%92.assets/image-20241205225832571.png)
...
**场景：**上个例子我们假设所有（自变量和因变量）数据都是来自独立同分布的正态分布，但实际由于数据来源不同，观测数据可能服从不同的正态分布，假设有 $N$ 个观测数据 $(x_i,y_i)$，每个观察数据都来自于不同的正态分布，即 
