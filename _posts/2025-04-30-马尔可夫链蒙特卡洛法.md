...
一般的 MCMC 教程上都是以积分计算为例子，我一度以为 MCMC 的核心是计算积分，对应到贝叶斯统计中是计算后验分布 $\pi(\theta|x) = \frac{p(x|\theta)\pi(\theta)}{\int p(x|\theta)\pi(\theta) dx}$ 中分母的这个积分，直到我弄懂了 MCMC 可以直接对后验分布抽样弄明白，**它的核心在抽样**。
