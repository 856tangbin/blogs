...
$$
  ...
  这样的建议分布称为 Metropolis 选择，也是 Metropolis-Hastings 算法最初采用的建议分布。这可以将接受概率中的建议分布抵消掉，简化接受分布。例如多元正态分布、$q(x,x') = q(\lvert x-x'\rvert)$ 等都是对称分布。**它的特点是** $x'$ 与 $x$ 接近时接受概率高，向附近转移的可能性大。
