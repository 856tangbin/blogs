---
title: 计算遗传率：LDSC 学习与实践笔记
description: 用 LDSC（LD Score Regression）从 GWAS 汇总统计量估计遗传率；包含原理、环境配置（Python 2.7）、坐标转 rsID、与参考面板合并、生成 .sumstats.gz，以及微生物组与代谢组两个性状的结果解读。
author: 吴玥冉
date: 2026-03-01 12:00:00 +0800
categories: [统计遗传学, 遗传率估计]
tags: [LDSC, 遗传率, 全基因组关联分析, 微生物组, 汇总统计量]
pin: false
math: true
mermaid: true
---
